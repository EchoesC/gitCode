...
//作者，最少一位
var CHKAUTHOR = "^[a-zA-Z0-9\u4e00-\u9fa5]{1,}$";
//内容，最少一位
var CHKCONTENT = "^[a-zA-Z0-9\u4e00-\u9fa5\u3002|\uff1f|\uff01|\uff0c|\u3001|\uff1b|\uff1a|\u201c|\u201d|\u2018|\u2019|\uff08|\uff09|\u300a|\u300b|\u3008|\u3009|\u3010|\u3011|\u300e|\u300f|\u300c|\u300d|\ufe43|\ufe44|\u3014|\u3015|\u2026|\u2014|\uff5e|\ufe4f|\uffe5]{15,}$";
//推荐程度，最少一位，只能是数字
var CHKISREMOD = "^[0-9]{1,}$";
function chkTitle() {
	//获取用户名输入信息
...
function chkAuthor() {
	...
	//获取span框
	var span = document.getElementById("resultAuthor");
...
function chkIsr() {
	//获取用户名输入信息
	var isrEle = document.getElementById("isr");
	var isr = isrEle.value;
	//获取span框
	var span = document.getElementById("resultIsr");
	var reg = new RegExp(CHKISREMOD);
	if (reg.test(isr)) {
		span.innerHTML = "<img src='check_right.png'>"
		span.style.position = "relative";
		span.style.top = "-27px";
		...
		span.innerHTML = "推荐程度只能为数字，并且最少一位";
		span.style.color = "red";
		//清空文本框
		isrEle.value = "";
		//重新聚焦
...
<body>
	<article class="page-container">
		<form action="articleupdate.do?id=${article.id }" method="post"
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="${article.author }" placeholder="" id="author"
						name="author" onblur="chkAuthor()"><span
						id="resultAuthor"></span>
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3"><span
				...
				<div class="formControls col-xs-8 col-sm-9" style="height: 200px;">
					<textarea type="textarea" class="" value="" placeholder="" id="content"
						name="content" style="height: 200px; width: 400px;" onblur="chkContent()">${article.content }</textarea>
						<span
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
						...
						id="datemax" class="input-text Wdate" name="crtime"
						style="width: 120px;" value="${article.crtime }">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<span class="select-box" style="width: 150px;"> <select
						class="select" name="channel" size="1">
							<c:forEach items="${chans }" var="chan">
								<c:if test="${article.channel==chan.id }">
									<option value="${chan.id }" selected="selected">${chan.cname }</option>
								</c:if>
								<c:if test="${article.channel!=chan.id }">
									<option value="${chan.id }">${chan.cname }</option>
								</c:if>
							</c:forEach>
			...
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3">推荐程度：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="${article.isremod }" placeholder="" id="isr"
						name="isremod" onblur="chkIsr()"><span
						id="resultIsr"></span>
				</div>
			</div>
			<div class="row cl">
